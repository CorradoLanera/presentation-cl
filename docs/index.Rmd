...
- A brief history of <img src="images/logopedianet.png", width = "20%", align = "center">
...
# #1 insights 
  - Single occurrence in a life-time (time-independent)
...
#### Classification of patients' records into the following six hierarchical classes:
...
### Notes:
- Label 4 and 5 can coexist 
- Label 5 is time-dependent: three OMA events within 6 months, or 4+ OMA events in 12 months.
- Label 5: if the pediatrician explicitly reports that OMA is recurrent, the record has to be marked as well despite of the timing.
...
For the current project, they have provided us an extraction of the db after the same search string filtering, on all diagnoses, sign-and-symptoms, and diaries free-text fields. Whatever excluded should be considered of label 0 (non-otitis)
...
- Multiple text-style (i.e., different pediatrics): tag? 

- Patent cannot be identified across pediatricians

- Possible multi-label classification: time-dependent, post-process layer?

- Hierarchical models: multi-stage?

- Data imbalance: all stages are possibly biased.
...
# .center[<mark>???BOIA IL FLOW-CHART???</mark>]
# .center[<mark>???SAREBBE CARINO???</mark>]
# .center[<mark>???LO AVREI ANCHE PROMESSO???</mark>]
# .center[<mark>???METTO STA SEZIONE???</mark>]
...
# Weights and features definition/selection
...
- **Diagnoses attribution**
  - discovered by augmentation and enrichment only?
...
# Class imbalance and learning algorithms
...
- Stage-chain performance analyses
  ...
  - each stage from the second to the fourth will be compared to the performance obtained by the model on _its own gold test set_, i.e. the one consisting of the records corresponding to 100% correct previous stage prediction. 
...
- Train, dev and test sets distributions are biased from the full  <img src="images/logopedianet.png", width = "20%", align = "center"> db by the search string matching
...
- Design/mutate/adapt the system to active learning

- Performance exploration of RNN algorithm<sup>*</sup>
...
.footnote[
<small><small>
<sup>\*</sup> Disclaimer: I have never implemented a RNN yet. 
</small></small>
]
---
class: center, middle, inverse
...
# <mark>Thank you</mark><br> for your attention
