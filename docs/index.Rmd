...
- Machine learning strategy (draft)
  1. global flow-chart
  1. preprocessing
  1. feature definition and weigths
  1. class imbalance?
  1. training flow
...
---
class: center, middle, inverse
# Detection and Classification of Otitis from PediaNET free-text medical notes

---

# #1 Task

> Detection of cases of otitis in children from free-text notes only.
> Record level.

> Classify patient records into the following hierarchies (only):
  - not an otitis case (label = 0)
  - otitis not media (label = 1)
      - media not acute (label = 2)
        - acute not recurrent and without perforation (label = 3)
          - acute with perforation (label = 4)
          - acute recurrent (label = 5)
          
### Notes:
- label 4 and 5 can cohesists 
- label 5 is time-dependent: three OMA events within 6 months, or 4+ OMA events in 12 months.
- label 5: if the pediatrician explicitely reports that OMA is recurrent, the record has to be marked as well despite of the timing.
...
# Feature definition and weigths
